...
def get_dynamic_suggestions(index: ChatIndex, source_name: str) -> Dict[str, List[Dict[str, str]]]:
    ...
    if "sample_chat_3" in src_lower or any("washing machine" in m["message"].lower() for m in index.messages[:10]):
        ...
        # Dynamic sender & temporal discovery from active archive
        p1 = senders[0] if len(senders) > 0 else "Kabir"
        p2 = senders[1] if len(senders) > 1 else "Meera"
        p3 = senders[2] if len(senders) > 2 else "Priya"

        # Discover months from timestamps
        months = []
        for dt in index.timestamps:
            m_name = dt.strftime("%B")
            if m_name not in months:
                months.append(m_name)

        m1 = months[0] if len(months) > 0 else "March"
        m2 = months[min(1, len(months) - 1)] if len(months) > 0 else "May"
        m3 = months[-1] if len(months) > 0 else "June"

        return {
            "meaning": [
                {"title": "Trip & Destination Decision", "query": "When did we decide on the mountain holiday?", "icon": "🏔️", "desc": "Zero-overlap semantics"},
            ...
            "person": [
                {"title": f"What did {p1} say", "query": f"what did {p1} say about the resort rooms", "icon": "👤", "desc": f"Author: {p1}"},
                {"title": f"What did {p2} say", "query": f"what did {p2} say about flight tickets", "icon": "👤", "desc": f"Author: {p2}"},
                {"title": f"What did {p3} say", "query": f"what did {p3} say about packing", "icon": "👤", "desc": f"Author: {p3}"}
            ],
            "time": [
                {"title": f"Early {m1} Discussions", "query": f"what did we discuss in early {m1}", "icon": "📅", "desc": f"Time: Early {m1}"},
                {"title": f"Mid {m2} Timeline", "query": f"what happened in the chat during {m2}", "icon": "📅", "desc": f"Time: {m2}"},
                {"title": f"Discussions in {m3}", "query": f"what was discussed in {m3}", "icon": "📅", "desc": f"Time: {m3}"}
            ],
            "decision": [
                {"title": "Trip Destination Finalized", "query": "was the destination ever finalized", "icon": "🎯", "desc": "Final decision"},
                {"title": "Travel Budget per Head", "query": "what is the budget per person for travel and stay", "icon": "🎯", "desc": "Budget outcome"},
                {"title": "Rooming Arrangement", "query": "how are they splitting up the sleeping arrangements", "icon": "🎯", "desc": "Sleeping outcome"}
            ]
        }
...
async def upload_custom_chat(file: UploadFile = File(...)):
    ...
    raw_name = file.filename or "uploaded_chat.txt"
    ...
    try:
        ...
        senders = sorted(list(set(m["sender"] for m in _index.messages if m["sender"] and m["sender"] != "System")))
        return {
            "status": "success",
...
async def api_search(
    ...
    # BM25 baseline top matches (strictly positive scores only)
