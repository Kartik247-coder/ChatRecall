...
1. `ChatIndex` - 20 edges
2. `RetrievalEngine` - 12 edges
...
Cohesion: 0.16
Nodes (16): get, display_results(), main(), Interactive Terminal Interface for ChatRecall…, Thread Context Engine for ChatRecall ====================================…, ThreadContextBuilder, ChatIndex, main() (+8 more)
...
- **Why does `ChatIndex` connect `ChatIndex` to `MessageEmbedder`, `retrieve.py`, `RetrievalEngine`?**
  _High betweenness centrality (0.130) - this node is a cross-community bridge._
- **Why does `MessageEmbedder` connect `MessageEmbedder` to `ChatIndex`?**
  _High betweenness centrality (0.086) - this node is a cross-community bridge._
- **Are the 7 inferred relationships involving `ChatIndex` (e.g. with `run_benchmark()` and `main()`) actually correct?**
  _`ChatIndex` has 7 INFERRED edges - model-reasoned connections that need verification._
