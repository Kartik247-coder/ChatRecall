...
## Corpus Check
- 30 files · ~133,438 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 189 nodes · 300 edges · 20 communities (12 shown, 6 thin omitted)
- Extraction: 93% EXTRACTED · 7% INFERRED · 0% AMBIGUOUS · INFERRED: 20 edges (avg confidence: 0.94)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `7b728757`
...
7. `run_demo_benchmark()` - 7 edges
8. `QueryRouter` - 7 edges
9. `get_services()` - 7 edges
10. `upload_custom_chat()` - 7 edges
...
Cohesion: 0.16
Nodes (19): get, post, display_results(), main(), Interactive Terminal Interface for ChatRecall…, Thread Context Engine for ChatRecall ====================================…, ThreadContextBuilder, ChatIndex (+11 more)
...
Cohesion: 0.12
Nodes (17): datetime, Enum, QueryPlan, QueryRouter, Multi-Strategy Query Router & Retrieval Engine for ChatRecall…, Expands query with conversational domain synonyms., Dynamically parses time expressions relative to active archive timestamps., StrategyType (+9 more)
...
### Community 6 - ".build_or_load"
Cohesion: 0.18
Nodes (13): expand_query(), search(), Loads chat data (from .json or .txt) and cached embeddings, or builds them if…, load_custom_txt_to_json(), parse_chat_txt(), parse_timestamp(), Any, Text Chat Parser for ChatRecall =============================== Parses raw… (+5 more)
...
- **Why does `ChatIndex` connect `ChatIndex` to `retrieve.py`, `MessageEmbedder`, `.build_or_load`, `run_demo_eval.py`, `run_eval.py`?**
  _High betweenness centrality (0.121) - this node is a cross-community bridge._
- **Why does `MessageEmbedder` connect `MessageEmbedder` to `ChatIndex`, `.build_or_load`?**
  _High betweenness centrality (0.079) - this node is a cross-community bridge._
- **Why does `RetrievalEngine` connect `ChatIndex` to `retrieve.py`, `.build_or_load`, `run_demo_eval.py`, `run_eval.py`, `.search`?**
